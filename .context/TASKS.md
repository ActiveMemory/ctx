...
- [x] Expand Backend.Models(ctx context.Context) ([]string, error) interface method to give ctx ai ping its 'first model listed' enrichment per specs/ctx-ai-backend.md §Behavior. Current ping only confirms reachability; spec requires reporting the first model from /v1/models. Implementation sketch: (1) add Models method to Backend interface in internal/backend/types.go; (2) implement on *openAICompat in openaicompat.go by reusing the GET /v1/models call and parsing the data[].id array; (3) wire into internal/cli/ai/cmd/ping/run.go to surface 'backend X reachable, first model: Y'; (4) update write/ai InfoPingOK signature and YAML. Pulled out of Phase BE Task 7 closeout (Task 7 row in TASKS.md flags this). #session:8f8cab12 #branch:feat/vllm-integration #commit:671473c0 #added:2026-05-23-171359
  ...
  `Backend` interface; implemented on `*openAICompat` by issuing
  `GET /v1/models` and parsing `data[].id` into an ordered slice
  via a new `modelsResponse` wire shape. Empty-list responses
  surface as new sentinel `ErrEmptyModels` + constructor
  `EmptyModels(name)` so callers can distinguish "reachable but
  unusable" from a transport failure. `ctx ai ping` now calls
  `b.Models(ctx)` after a successful `Ping`, surfaces the first
  model via the extended `writeAI.InfoPingOK(cmd, name, model)`
  signature; YAML `write.ai-ping-ok` template now reads
  `backend %q reachable; first model: %q`. vllm inherits Models
  from the embedded openAICompat unchanged (cold-start retry
  remains scoped to Ping; once Ping returns, the listener is up,
  so a follow-up Models call doesn't need its own retry). New
  test rows: openaicompat Models {happy, order-preserved,
  empty, unhealthy, bad-JSON, unreachable} and a ping
