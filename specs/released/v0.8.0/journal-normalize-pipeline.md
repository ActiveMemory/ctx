...
normalizeContent(content, fencesVerified)
  1. stripFences(content)        — remove all ``` / ~~~ fence lines
  2. wrapToolOutputs(content)    — wrap Tool Output sections in fenced code blocks
  3. Line-by-line pass (skip lines inside fenced code blocks):
     ...
     c. Demote non-turn-header headings to bold (## Foo → **Foo**)
     d. Insert blank line before list items when previous line is non-empty
     e. Strip bold from tool-use lines (** → 🔧)
     f. Escape glob stars outside indented lines
     g. Replace inline code spans with angle brackets (` → ", < → &lt;)
```
...
tolerating gaps in turn numbering (e.g., 348 → 350).
...
|---|---|---|---|
| `<` `>` | → `&lt;` `&gt;` | Left to fence wrapping | → `&lt;` `&gt;` + backtick→quote |
...
`config.RecallMaxTitleLen = 75` — truncates on word boundary (last space
...
- `internal/config/regex.go` — `RegExClaudeTag`, `RegExInlineCodeAngle`
- `internal/config/limit.go` — `RecallMaxTitleLen = 75`
- `internal/config/tpl_journal.go` — `TplZensicalExtraCSS`, `JournalExtraCSS`
- `internal/cli/journal/normalize.go` — `wrapToolOutputs`, `stripPreWrapper`, inline code handling, H1 sanitization
- `internal/cli/journal/parse.go` — title sanitization (entities, backtick/hash strip)
- `internal/cli/journal/generate.go` — `extra_css` TOML emission
- `internal/cli/journal/run.go` — write `docs/stylesheets/extra.css`
- `internal/cli/recall/slug.go` — `cleanTitle` Claude tag strip + 75-char truncation
