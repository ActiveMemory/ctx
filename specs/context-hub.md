# Spec: ctx Hub (`ctx serve --hub` + `ctx connect`)
...
ctx serve --hub --port 9900 --tls-cert cert.pem --tls-key key.pem
...
#   → writes to .context/hub/
#   → records last-seen sequence

# 4. Listen for real-time updates (long-lived)
ctx connect listen
#   → gRPC server-stream, writes new entries to .context/hub/
...
ctx agent --include-hub    # local + shared knowledge
ctx agent --include-hub --hub-budget 3000  # custom shared budget
```
...
3. ctx connect sync
   └── pulls all matching entries from hub → .context/hub/
