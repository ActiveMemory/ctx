# Decision: Federation Strategy for ctx Hub
...
The ctx Hub ([context-hub.md](context-hub.md))
...
`ctx serve --hub` becomes a daemon:

```bash
# Start hub daemon (foreground, for dev/testing)
ctx serve --hub --port 9900 \
...
# On hub-1:
ctx serve --hub --port 9900 --peers hub-2:9900,hub-3:9900 --daemon
# On hub-2:
ctx serve --hub --port 9900 --peers hub-1:9900,hub-3:9900 --daemon
# On hub-3:
ctx serve --hub --port 9900 --peers hub-1:9900,hub-2:9900 --daemon
...
- Data replication is simple sequence-based sync (append-only, no
  conflicts)
- `ctx serve --hub` gains `--daemon` and `--peers` flags
