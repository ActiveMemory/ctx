...
export default function Reminders({ dir }: { dir: string }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-6">
      <h1 className="mb-1 text-lg font-semibold text-ink">Reminders</h1>
      <p className="mb-4 text-xs text-muted">
        Session-scoped notes that surface verbatim at session start and
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" && text.trim()) {
              void run(() => ctxRemindAdd(dir, text.trim())).then(() =>
                setText(""),
              );
            }
          }}
      ...
      <div className="mb-4 flex items-center gap-2">
        ...
        <button
          onClick={() =>
            void run(() => ctxRemindDismiss(dir, target.trim())).then(() =>
              setTarget(""),
            )
          }
        ...
        <button
          onClick={() => void run(() => ctxRemindDismiss(dir, "all"))}
          disabled={busy}
          className="rounded-md border border-border bg-bg px-3 py-1.5 text-xs text-muted hover:border-accent hover:text-ink disabled:opacity-50"
        >
          Dismiss all
        </button>
      </div>
