...
var CtxAbsoluteStart = regexp.MustCompile(
	`^\s*(/home/|/tmp/|/var/)\S*/ctx(\s|$)`)

// AbsoluteSep matches absolute paths to ctx after command separator.
var AbsoluteSep = regexp.MustCompile(
	`(&&|;|\|\||\|)\s*` +
		`(/home/|/tmp/|/var/)\S*/ctx(\s|$)`)
...
var CtxRelativeSep = regexp.MustCompile(
	`(&&|;|\|\||\|)\s*` +
		`(\./ctx(\s|$)|\./dist/ctx)`)
