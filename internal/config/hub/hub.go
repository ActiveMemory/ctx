...
const (
	...
	// throttle plus next-day retry covers a cut-off pull (the
	// throttle marker is stamped unconditionally, so retry is
	// bounded to the next day's first session, not the next).
