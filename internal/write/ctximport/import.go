...
func NoEntries(cmd *cobra.Command, filename string) {
	...
	cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportNoEntries), filename))
}
...
func ScanHeader(cmd *cobra.Command, filename string, count int) {
	...
	cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportScanning), filename))
	cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportFound), count))
	cmd.Println()
}
...
func EntrySkipped(cmd *cobra.Command, title string) {
	...
	cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportEntry), title))
	cmd.Println(desc.TextDesc(text.TextDescKeyWriteImportClassifiedSkip))
	cmd.Println()
}
...
func EntryClassified(cmd *cobra.Command, title, targetFile string, keywords []string) {
	...
	cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportEntry), title))
	cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportClassified), targetFile, strings.Join(keywords, ", ")))
	cmd.Println()
}
...
func EntryAdded(cmd *cobra.Command, title, targetFile string) {
	...
	cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportEntry), title))
	cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportAdded), targetFile))
	cmd.Println()
}
...
func Summary(cmd *cobra.Command, result entity.ImportResult, dryRun bool) {
	...
	var summary string
	if dryRun {
		summary = fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportSummaryDryRun), total)
	} else {
		summary = fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportSummary), total)
	}

	var parts []string
	if result.Conventions > 0 {
		parts = append(parts, fmt.Sprintf(
			desc.TextDesc(text.TextDescKeyImportCountConvention), result.Conventions))
	}
	if result.Decisions > 0 {
		parts = append(parts, fmt.Sprintf(
			desc.TextDesc(text.TextDescKeyImportCountDecision), result.Decisions))
	}
	if result.Learnings > 0 {
		parts = append(parts, fmt.Sprintf(
			desc.TextDesc(text.TextDescKeyImportCountLearning), result.Learnings))
	}
	if result.Tasks > 0 {
		parts = append(parts, fmt.Sprintf(
			desc.TextDesc(text.TextDescKeyImportCountTask), result.Tasks))
	}
	if len(parts) > 0 {
	...
	if result.Skipped > 0 {
		cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportSkipped), result.Skipped))
	}
	if result.Dupes > 0 {
		cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyWriteImportDuplicates), result.Dupes))
	}
}
