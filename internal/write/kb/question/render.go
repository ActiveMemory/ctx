...
func renderRow(id string, row Row) string {
	...
	sb.WriteString(escapeCell(row.Question))
	sb.WriteString(marker.TableCellSep)
	sb.WriteString(escapeCell(row.WhyItMatters))
	sb.WriteString(marker.TableCellSep)
	sb.WriteString(escapeCell(row.WhatEvidenceWouldResolve))
	sb.WriteString(marker.TableCellSep)
	sb.WriteString(escapeCell(row.OpenedAt))
	sb.WriteString(marker.TableCellSep)
	sb.WriteString(escapeCell(
		strings.Join(row.RelatedEV, token.CommaSpace),
	))
