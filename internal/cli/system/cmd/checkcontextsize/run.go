...
func runCheckContextSize(cmd *cobra.Command, stdin *os.File) error {
	...
	if core.WrappedUpRecently() {
		core.LogMessage(logFile, sessionID, fmt.Sprintf("prompt#%d suppressed (wrapped up)", count))
	...
	default:
		core.LogMessage(logFile, sessionID, fmt.Sprintf("prompt#%d silent", count))
	}
...
func emitCheckpoint(cmd *cobra.Command, logFile, sessionID string, count, tokens, pct, windowSize int) {
	...
	content := core.LoadMessage("check-context-size", "checkpoint", nil, fallback)
	if content == "" {
		core.LogMessage(logFile, sessionID, fmt.Sprintf("prompt#%d silenced-by-template", count))
		return
	}
	...
	cmd.Println(msg)
	cmd.Println()
	core.LogMessage(logFile, sessionID, fmt.Sprintf("prompt#%d CHECKPOINT tokens=%d pct=%d%%", count, tokens, pct))
...
func emitWindowWarning(cmd *cobra.Command, logFile, sessionID string, count, tokens, pct int) {
	fallback := fmt.Sprintf("⚠ Context window is %d%% full (~%s tokens).\n"+
		...
		"or starting a fresh session with /ctx-wrap-up.", pct, core.FormatTokenCount(tokens))
	content := core.LoadMessage("check-context-size", "window",
		map[string]any{"Percentage": pct, "TokenCount": core.FormatTokenCount(tokens)}, fallback)
	if content == "" {
		core.LogMessage(logFile, sessionID, fmt.Sprintf("prompt#%d window-silenced pct=%d%%", count, pct))
		return
	}
	...
	cmd.Println(msg)
	cmd.Println()
	core.LogMessage(logFile, sessionID, fmt.Sprintf("prompt#%d WINDOW-WARNING tokens=%d pct=%d%%", count, tokens, pct))
	ref := notify.NewTemplateRef("check-context-size", "window",
		map[string]any{"Percentage": pct, "TokenCount": core.FormatTokenCount(tokens)})
...
func appendOversizeNudge() string {
	...
	content := core.LoadMessage("check-context-size", "oversize",
...
func emitBillingWarning(cmd *cobra.Command, logFile, sessionID string, count, tokens, threshold int) {
	...
	warnedFile := filepath.Join(core.StateDir(), "billing-warned-"+sessionID)
	...
	content := core.LoadMessage("check-context-size", "billing",
		map[string]any{"TokenCount": core.FormatTokenCount(tokens), "Threshold": core.FormatTokenCount(threshold)}, fallback)
	if content == "" {
		core.LogMessage(logFile, sessionID, fmt.Sprintf("prompt#%d billing-silenced tokens=%d threshold=%d", count, tokens, threshold))
		core.TouchFile(warnedFile) // silenced counts as fired
		return
	}
	...
	core.TouchFile(warnedFile) // one-shot: mark as fired
	core.LogMessage(logFile, sessionID, fmt.Sprintf("prompt#%d BILLING-WARNING tokens=%d threshold=%d", count, tokens, threshold))
	ref := notify.NewTemplateRef("check-context-size", "billing",
		map[string]any{"TokenCount": core.FormatTokenCount(tokens), "Threshold": core.FormatTokenCount(threshold)})
