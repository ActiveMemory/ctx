...
func runMarkJournal(cmd *cobra.Command, filename, stage string) error {
	...
	if check {
		...
		cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyMarkJournalChecked), filename, stage, val))
		return nil
	}
	...
	cmd.Println(fmt.Sprintf(desc.TextDesc(text.TextDescKeyMarkJournalMarked), filename, stage))
	return nil
}
