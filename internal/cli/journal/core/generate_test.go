...
func TestGenerateZensicalToml_WithAllNav(t *testing.T) {
	entries := []JournalEntry{{Filename: "a.md", Title: "A", Date: "2026-01-01"}}
	topics := []TopicData{{Name: "t", Entries: entries}}
	keyFiles := []KeyFileData{{Path: "f.go", Entries: entries}}
	sessionTypes := []TypeData{{Name: "feature", Entries: entries}}
