...
project root, the bootstrap command inherits the external path automatically —
no manual CLAUDE.md editing needed.

Every nudge (context checkpoint, persistence reminder, etc.) also includes a
`Context: /home/user/repos/myproject-context` footer, so the agent stays
anchored to the correct directory even in long sessions.
