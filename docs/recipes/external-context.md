...
!!! warning "Boundary Validation"
    ...
    every ctx command needs `--allow-outside-cwd` — unless you persist the
    setting in `.contextrc` (next step).
...
!!! tip "Commit `.contextrc`"
    ...
    path — and it lets teammates share the same configuration.

!!! note "Boundary override"
    When `context_dir` in `.contextrc` points outside the project root, you
    still need `--allow-outside-cwd` on each command, or set it as a shell
    alias (see below). The `.contextrc` replaces the `--context-dir` flag but
    does not override the boundary check.
...
If you use the same external directory regularly and want to bake in
`--allow-outside-cwd`:
...
* **Don't forget the boundary flag**. The most common error is
  `Error: context directory is outside the project root`. Add
  `--allow-outside-cwd` or use a shell alias.
