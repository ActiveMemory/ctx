...
| **Sonnet OFF plan, cheaper implementation**<br>~$4.92 · ~50m15s | Sonnet (compression OFF) found and specified the smaller wiring task.                              | A cheaper model implemented the ratified task list. | Worth repeating as a follow-up experiment. Not a default rule.                                                             |
| **Sonnet ON plan, cheaper implementation**<br>~$4.9 · composite | Sonnet (compression ON) planned the larger parser task. | Haiku implemented the ratified task list. | Acceptable, but inherited the larger premise. |
...
| Sonnet compression OFF / cheaper implementation | Acceptable bounded-executor trial        | The cheaper model followed the ratified plan, but tests were thinner. This supports retesting cheap execution after task shape is fixed. |
...
| Sonnet compression ON / cheaper implementation  | Acceptable, but inherited larger premise | The implementation stayed inside the earlier parser-work shape.                                                                          |
...
| Haiku YOLO                                      | Incomplete                               | Fixed `"1, 2, 3"` but kept duplicated parsing and missed whitespace-only `[0]`.                                                          |
